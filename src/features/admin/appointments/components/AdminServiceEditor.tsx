...
export default function AdminServiceEditor({
    ...
    const [state, formAction, pending] = useActionState(
        updateAppointmentServicesAction,
        {},
    );
    const inferred = useMemo(
        () => inferAdminServiceSelections(booking),
        [booking],
    );
    const serviceLineItems = booking.lineItems.filter(
        (item) => item.itemType !== "discount",
    );
    const discountItem =
        booking.lineItems.find((item) => item.itemType === "discount") ?? null;
    const currentEligibleSubtotal = roundCurrency(
    ...
    const discountPercentage = discountItem
        ? (parseAdminDiscountPercentage({
              label: discountItem.label,
              fallbackAmount: Math.abs(discountItem.lineTotal),
              subtotal: currentEligibleSubtotal,
          }) ?? 0)
        : 0;
    ...
    const selectedOption = getServiceOption(
        selectedService,
        ...
        <section className="overflow-hidden rounded-3xl border border-dark-green/20 bg-surface shadow-sm">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                        {serviceLineItems.length ? (
                            serviceLineItems.map((item) => (
                                <span
                                    key={item.id}
                                    className="rounded-full border border-border/60 bg-surface px-3 py-1.5 text-sm font-medium text-foreground"
                                >
                                    {item.label}
                                </span>
                            ))
            ...
            <form action={formAction} className="p-5 sm:p-7">
                ...
                <input
                    ...
                    value={
                        fullService ? (selections.serviceOptionId ?? "") : ""
                    }
                />
                <input
                    type="hidden"
                ...
                <div className="mb-5">
                    ...
                    <p className="mt-1 text-sm text-muted">
                        Review the live price below before saving.
                    </p>
                </div>
                ...
                <div className="grid gap-4 xl:grid-cols-2">
                    ...
                    <AppSelect
                        ...
                        options={(
                            ...
                            .map((option) => ({
                                value: option.id,
                                label: `${buildServiceOptionLabel(selectedService, option)} · ${formatMoney(option.price)}`,
                            }))}
                    />
                ...
                {!inferred.reliable ? (
                    <p className="mt-4 rounded-2xl border border-border/60 bg-background p-4 text-sm text-muted">
                        The saved service labels could not be matched safely.
                        The existing booking will remain unchanged until its
                    ...
                    <Price
                        label={
                            discountPercentage > 0
                                ? `Updated total (${discountPercentage}% off)`
                                : "Updated total"
                        }
                        value={updatedPricing.total}
                        emphasized
                    />
                    <div>
                        ...
                        <p
                            className={`mt-2 flex items-center gap-1 text-lg font-semibold ${difference > 0 ? "text-dark-green" : difference < 0 ? "text-foreground" : "text-muted"}`}
                        >
                            {difference > 0 ? (
                ...
                <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <p className="text-sm text-muted">
                        {hasChanges
                            ? "Changes are only applied after you save."
                            : "Current selections are loaded from this booking."}
                    </p>
                    <button
                        type="submit"
...
function Price({
    ...
    return (
        <div>
            ...
            <p
                className={`mt-2 font-semibold text-foreground ${emphasized ? "text-2xl" : "text-lg"}`}
            >
                {formatMoney(value)}
            </p>
        </div>
