...
export default function AdminAppointmentsPage({ appointments, search, status, view, nowIso }: { appointments: AdminAppointmentListItem[]; search: string; status: string; view: AdminAppointmentView | null; nowIso: string }) {
    ...
    const isPast = (booking: AdminAppointmentListItem) => Boolean(booking.startsAt && +(booking.endsAt ? new Date(booking.endsAt) : new Date(booking.startsAt)) < now);
    const isCurrent = (booking: AdminAppointmentListItem) => {
        if (!(booking.status === "confirmed" || booking.status === "cancellation_requested") || !booking.startsAt) return false;
        const startsAt = +new Date(booking.startsAt);
        const endsAt = booking.endsAt ? +new Date(booking.endsAt) : startsAt + 90 * 60_000;
